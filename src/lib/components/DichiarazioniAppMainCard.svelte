<script>
  ...
  onMount(async () => {
    ...
    numAttuali = data.find(
      (d) => d.indicatore == "num_dichiarazioni_app_pub"
    ).valore;

    enti = data.find(
      (d) => d.indicatore == "num_enti_dichiarazioni_app_pub"
    ).valore;
...
<div class="breadcrumb mt-5">
  <p><a href="/dichiarazione" title={$t("dicMain.prevCrumb")} class="fw-bold text-decoration-underline" >{$t("dicMain.prevCrumb")}</a> / {$t("dicMain.APP-currentCrumb")}</p>
</div>
...
<div class="d-flex flex-wrap justify-content-between mt-3 mb-5">
  <div class="col-12 col-lg-6 flex-wrap">
    ...
    <p>
      {@html $t("dicMain.APP-description", {
        break: "<br/>",
        doubleBreak: "<br/><br/>",
        anno: annoRiferimento,
        annoDopo: +annoRiferimento + 1
      })}
    </p>
  </div>

<script lang="ts">
  ...
  const smallColumns = [
    {
      field: `des_success_criteria_${$locale}`,
      label: $t("erroriTableApp.criterio"),
      format: (value: any) => value.substring(0, 8),
    },
    {
      field: "cod_livello_conformita",
      label: $t("erroriTableApp.livello")
  ...
  onMount(async () => {
    const rs = await fetch("/data/mona_siti_errori_distr_conformita.json");
    ...
    for (let i = 0; i < response.data.length; i++) {
      for (let z = 0; z < arrayIcone.length; z++) {
        if (
          response.data[i].des_success_criteria_it.substring(2, 8).trim() ==
          arrayIcone[z].id
        ) {
...
{#if !loading}
  ...
  >
    <div slot="didascaliaSlot" class="didascalia">
      {@html $t("erroriTableApp.descriptionSitiPt1", {break: "<br/>"})}
      <a
      ...
      {$t("erroriTableApp.descriptionSitiPt2")}
    </div>
  </DataTable>
{/if}

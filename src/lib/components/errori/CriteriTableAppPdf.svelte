<script lang="ts">
  ...
  onMount(async () => {
    const rs = await fetch("/data/mona_siti_pdf_errori_distr_conformita.json");
    ...
    response.data.forEach((item) => {
      const targetId = item.des_success_criteria_it.substring(3, 9).trim();
...
<div bind:this={pdfRef} class="my-3">
  {#if !loading}
    <DataTable
      ...
      title={$t("erroriTableApp.titlePdf")}
      didascalia={true}

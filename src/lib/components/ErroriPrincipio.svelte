<script>
  ...
  let principleTextMap = {
    Percepibile:
    "https://www.w3.org/Translations/WCAG21-it/#perceivable",
    Utilizzabile:
      "https://www.w3.org/Translations/WCAG21-it/#operable",
    Robusto:
      "https://www.w3.org/Translations/WCAG21-it/#robust",
    Comprensibile:
      "https://www.w3.org/Translations/WCAG21-it/#understandable",
  };
  ...
  onMount(async () => {
    ...
    ordinePrincipi.forEach((op) => {
      orderedErrors.push(
        erroriRiscontratiPrincipio.find((erp) => erp.des_principio == op)
      );
    });
...
<div class="backgroundLightBlue pt-5 pb-3 px-xxl-5">
  <div class="container">
    <div class="text-center ">
      <h3 class="h3 greyText pb-3">Distribuzione errori per principio</h3>
    </div>
    <p class=" mx-3 mx-lg-5 px-lg-5 text-center">
      Distribuzione degli errori rilevati tramite il sistema automatizzato
      all’interno dei quattro principi previsti dalle
      <a
        ...
        {#each erroriRiscontratiPrincipio as item}
          <div class="col-12 col-md-6 col-xl-3 my-4">
            <p class="h4 d-inline-flex blueText">
              ...
              href={principleTextMap[item.des_principio]}
              title="Il link si apre in una nuova finestra"
              target="_blank"
              rel="noreferrer"
              >{item.des_principio} <Icon
                name="it it-external-link"
                size="sm"
                customClass="mb-1"
                variant="primary"
